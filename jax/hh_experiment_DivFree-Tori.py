...
from hh_experiment_setup import runHelmholtzExperiment,runHelmholtzExperiment3
import optax 
...
seed = 1231231#np.random.randint(2**32)
...
params = mlp.init(mkey,periodic(x[1:]))
scale = 1.4
params = jax.tree_map(lambda x: x*scale, params)
params = params.unfreeze()['params']

func_mlp = lambda x,params: mlp.apply({'params':params}, periodic(x))
...
opt = optax.adam(learning_rate=7e-4,b1=0.9,b2=0.99)
